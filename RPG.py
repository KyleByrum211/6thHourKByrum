...
def EnemyCheck():
    ...
    elif floorLevel < FinalFloor:
        Cahnce = random.randint(1, 12)
    else:
        Chance = 1
    if Chance == 1:
        Enemy = "Slime"
        print("You have encountered a slime.")
        EnemyAtkType = "Strength"
        ...
        EnemyAtkType = "Magic"
        EnemySpeed = random.randint(3, 5) + EnemyLevel
        EnemyStrength = random.randint(1, 2) + EnemyLevel
        EnemyDefense = random.randint(2, 3) + round(EnemyLevel / 2)
        EnemyMagic = random.randint(4, 7) + EnemyLevel
        EnemyAccuracy = 0.9
        EnemyHealth = 11 + EnemyLevel
    elif Chance == 8:
        Enemy = "Flamingo"
        print("You have encountered a flamingo.")
        EnemyAtkType = "Strength and Magic"
        EnemySpeed = random.randint(2, 4) + EnemyLevel
        EnemyStrength = random.randint(4, 6) + EnemyLevel
        EnemyDefense = random.randint(1, 3) + EnemyLevel
        EnemyMagic = random.randint(3, 5) + EnemyLevel
        EnemyAccuracy = 0.9
        EnemyHealth = 11 + EnemyLevel
    elif Chance == 9:
        Enemy = "Really Angry Magic Cat"
        print("You have encountered an angry cat that knows magic.")
        EnemyAtkType = "Magic"
        EnemySpeed = random.randint(5, 7) + EnemyLevel
        EnemyStrength = random.randint(1, 2) + math.floor(EnemyLevel * 0.4)
        EnemyDefense = random.randint(1, 3) + math.ceil(EnemyLevel * 0.67)
        EnemyMagic = random.randint(6, 9) + EnemyLevel
        EnemyAccuracy = 0.95
        EnemyHealth = 14 + EnemyLevel
    elif Chance == 10:
        Enemy = "Wolves"
        print("You have encountered two different wolves.")
        EnemyAtkType = "Strength"
        EnemySpeed = random.randint(7, 10) + EnemyLevel
        EnemyStrength = random.randint(2, 4) + math.floor(EnemyLevel * 0.3)
        EnemyDefense = random.randint(2, 4) + math.ceil(EnemyLevel * 0.9)
        EnemyMagic = random.randint(1, 2) + EnemyLevel
        EnemyAccuracy = 0.95
        EnemyHealth = 14 + EnemyLevel
    elif Chance == 11:
        Enemy = "Warded Penguin"
        print("You have encountered a magic resistant penguin.")
        EnemyAtkType = "Strength"
        EnemySpeed = random.randint(2, 4) + EnemyLevel
        EnemyStrength = random.randint(5, 8) + EnemyLevel
        EnemyDefense = random.randint(1, 3) + math.ceil(EnemyLevel * 0.1)
        EnemyMagic = 1 + math.floor(EnemyLevel * 0.1)
        EnemyAccuracy = 0.95
        EnemyHealth = 20 + (EnemyLevel * 2)
    elif Chance == 12:
        Enemy = "Ghostly Armour"
        print("You have encountered a floating set of metal armour.")
        EnemyAtkType = "Defense"
        EnemySpeed = 1 + math.floor(EnemyLevel * 0.3)
        EnemyStrength = random.randint(1, 2) + math.floor(EnemyLevel * 0.5)
        EnemyDefense = random.randint(8, 10) + math.ceil(EnemyLevel * 2)
        EnemyMagic = random.randint(2, 5) + EnemyLevel
        EnemyAccuracy = 0.8
        EnemyHealth = 17 + EnemyLevel
    if Enemy:
        print(
...
def doAttack(Damage, EnemyDamage, AttackType):
    ...
    if Speed > EnemySpeed:
        ...
        if EnemyHealth > 0:
            ...
            if Enemy == "Wolves":
                timesToAttack = random.randint(3, 5)
                for _ in range(timesToAttack):
                    enemyAttacks(EnemyDamage)
    else:
        ...
        if Health > 0:
            playerAttacks(Damage, AttackType)
    if Enemy == "Wolves":
        print(f"The {Enemy} attacked {timesToAttack} times!")
...
def useSpell():
    ...
    ReversalRed = Magic >= 20 and weapon == "Cursed Energy" and Level >= 3
    AmplifiedBlue = Magic >= 10 and weapon == "Cursed Energy" and Level >= 2
    ...
    Kamehameha = Magic >= 10 and Strength >= 10 and weapon == "Ki" and Level >= 2
    AngryKame = Magic >= 30 and weapon == "Ki" and Level >= 5
    ...
    if Infinity:
        ...
        time.sleep(0.2)
    if ReversalRed:
        print("ReversalRed (Red)")
        time.sleep(0.2)
    if AmplifiedBlue:
        print("AmplifiedBlue (Blue)")
        time.sleep(0.2)
    if HollowPurple:
    ...
    if Spell.lower() == "crit" and CritSpell:
        ...
        if trySpell == "y":
            if Mana >= 3:
                ...
                print("You do not have enough mana.")
    elif Spell.lower() == "black" and BlackHoleSpell:
        print("You selected the Black Hole Spell.")
        print("This spell kills any non-boss enemy but has a chance to backfire on you.")
        trySpell = input(f"This spell costs {math.ceil(maxMana / 2)} mana. Would you like to use it? (y): ")
        if trySpell == "y":
            if EnemyAtkType != "Boss":
                if Mana >= math.ceil(maxMana / 2):
                    Mana -= math.ceil(maxMana / 2)
                    UsedSpell = True
                    EnemyHealth = 0
                    if random.randint(1, 5) == 1:
                        print("The black hole backfired on you.")
                        Health = 0
                else:
                    print("You do not have enough mana.")
            else:
                print("You cannot use black hole on a boss.")
        ...
        if trySpell == "y":
            if Mana >= 10:
                ...
                UsedSpell = True
                MgcDamageSpell = True
                print("You blasted an energy beam.")
                Damage = (Strength + Magic) - EnemyDefense
        else:
            print("You have no mana.")
    elif Spell.lower() == "akame" and AngryKame:
        trySpell = input("This spell costs 10 mana. Would you like to use it? (y): ")
        if trySpell == "y":
            if Mana >= 20:
                Mana -= 20
                UsedSpell = True
                MgcDamageSpell = True
                print("YOU HEARTLESS BASTARD!!!")
                Damage = (Strength + Magic) * 2 - EnemyDefense
...
def statuses():
    ...
    if EnemyPoisoned:
        if poisonTurn is not None and poisonTurn == Turn:
            ...
            print(f"They took 1 damage. They are now at {EnemyHealth} health.")
...
def EnemyDamageCalculation():
    if EnemyAtkType == "Strength":
        EnemyDamage = EnemyStrength - Defense
    elif EnemyAtkType == "Magic":
        EnemyDamage = EnemyMagic - math.floor(Defense / 2)
    elif EnemyAtkType == "Speed":
        if (EnemySpeed / 2) >= Defense:
            EnemyDamage = (EnemySpeed * 2) - Defense
        else:
            EnemyDamage = 1
    elif EnemyAtkType == "Strength and Magic":
        EnemyDamage = (EnemyMagic + EnemyStrength) - math.floor(Defense * 1.5)
    elif EnemyAtkType == "Defense":
        EnemyDamage = math.ceil((EnemyDefense - Strength) * 1.5)
    elif EnemyAtkType == "Boss":
        if Enemy == "Giant Enemy Spider":
        ...
        elif Enemy == "Iron Wall":
            chance = random.randint(1, 3)
            if chance == 1:
                EnemyAccuracy = 0.9
                print(f"The {Enemy} intends to attack nuh uh.")
            elif chance == 2:
                EnemyAccuracy = 0.9
                EnemyDamage = EnemyMagic - math.floor(Defense / 2)
                print(f"The {Enemy} intends to attack with its magic.")
                ...
                EnemyAccuracy = 0.8
                print(f"The {Enemy} intends to attack with its defense.")
                EnemyDamage = math.ceil((EnemyDefense - Strength) * 1.5)


def startTurn():
    global item, weapon, Strength, Magic, Defense, Speed, Health, XP
    global nextLevel, Level, EnemyDamage, EnemiesLeft, CritChance, EnemyAccuracy, EnemyLevel, EnemyLevelPercent
    ...
    if Action.lower() == "atk":
        ...
        if Magic < 3:
            ...
            if Enemy == "Warded Penguin":
                print("You cannot use spells against the warded penguin.")
            else:
                useSpell()
...
def battleEnd():
    global Health, XP, startHealth, item, weapon, nextLevel, Level, EnemyLevel, money, maxMoney, EnemyLevelPercent
    global EnemyFrozen, EnemyBurned, EnemyPoisoned, EnemyIchored, Turn, EnemyAcidified, EnemyStunned, BossesKilled
    ...
    EnemyAcidified = False
    EnemyStunned = False
    EnemyLevelPercent += (random.randint(4, 7) / 10)
    EnemyLevel = math.floor(EnemyLevelPercent)
    ...
    if EnemyAtkType == "Boss":
        XP_Gained = math.floor(XP_Gained * 2)
    ...
    if Enemy == "Mimic":
        ...
        global potion
        print("The flamingo lord dropped some flamingo wings.")
        if item != None:
            if item == "Potion":
        ...
        print("The giant enemy spider increased your speed permanently!")
        Speed = math.floor(EnemySpeed * 1.1)
        time.sleep(0.5)
        print(f"You now have {Speed} speed!")
    else:
...
def die():
    ...
    global maxHealth, Speed, Strength, Defense, Magic, XP, Level, floorLevel
    time.sleep(1)
    if gotDeathBlowed:
    ...
    time.sleep(0.5)
    print(f"Bosses killed: {BossesKilled}")
    time.sleep(0.5)
    ...
    print(f"Level: {Level}")
    time.sleep(0.5)
    print(f"Dungeon floor: floor {floorLevel}")
...
def gameLoop():
    ...
    if floorLevel == 0:
        EnemyCheck()
    elif floorLevel >= 20 and EnemyLevel == 10:
        ...
        EnemyAccuracy = 0.85
        EnemyHealth = 40
    elif floorLevel >= 50 and EnemyLevel == 25:
        Enemy = "Flamingo Lord"
        ...
        EnemyAccuracy = 0.9
        EnemyHealth = 60
    elif floorLevel >= 80 and EnemyLevel == 40:
        Enemy = "Iron Wall"
        print("You have encountered a boss, the Iron Wall!.")
        time.sleep(0.5)
        EnemyAtkType = "Boss"
        EnemySpeed = 20 + random.randint(1, 5)
        EnemyStrength = 25 + random.randint(5, 10)
        EnemyDefense = 70 + random.randint(10, 20)
        EnemyMagic = 65 + random.randint(5, 10)
        EnemyAccuracy = 0.9
        EnemyHealth = 60
    elif floorLevel == FinalFloor:
        ...
        if random.randint(1, 10) == 1:
            ...
            if room == 1:
                ...
                print("There is a black hole at the enter of the room.")
                BlackHole = input("Do you wish to step into the black hole? (y): ")
                if BlackHole.lower() == "y":
                    print("Why would you willingly walk into a black hole? Did you have some sort of plan?")
                    time.sleep(0.5)
                    print("")
                    if BlackHoleSpell:
                        Health = 0
                        ...
                        die()
                        time.sleep(9)
                        print("It seems as though the gods of this world have given you another chance.")
                        time.sleep(1)
                        oldStrength = Strength
                        oldMagic = Magic
                        oldDefense = Defense
                        Strength = math.floor(Strength * 1.4)
                        Magic = math.floor(Magic * 1.4)
                        ...
                        EnemySpeed = random.randint(1, 2) + EnemyLevel
                        EnemyStrength = random.randint(1, 2) + EnemyLevel
                        EnemyDefense = random.randint(1, 3) + math.floor(EnemyLevel * 0.4)
                        EnemyMagic = random.randint(5, 8) + math.ceil(EnemyLevel * 1.4)
                        EnemyAccuracy = 0.9
                        EnemyHealth = 20 + (EnemyLevel * 2)
                        print("It looks like you're gonna have to fight the black hole.")
                        time.sleep(0.5)
                        print("You have been revived with ressurection stats for this fight.")
                        time.sleep(0.5)
        else:
            EnemyCheck()
    if Enemy:
...
if Start != "" and Start != "goku":
    ...
    time.sleep(0.5)
    print("")
if Start != "rieobjaobijrqebperjoberpbj𓅱mtgboitntiogn 𓅲rwogi nw evnrpovjmebpokvpok pmvveevve𓅱":
    while Health > 0 or floorLevel == FinalFloor:
        gameLoop()